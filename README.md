# test-dotnet-noinfra
test-dotnet-noinfra Dotnet service.
...
| ApplicationInsights__CloudRole          | Role used for filtering metrics    | no       |           |       | Set to `test-dotnet-noinfra-local` in docker compose files    |
...
`docker-compose -p test-dotnet-noinfra-test -f docker-compose.yaml -f docker-compose.test.yaml up`
...
`docker-compose -p test-dotnet-noinfra-test -f docker-compose.yaml -f docker-compose.test.watch.yaml up`
...
```
variableGroups:
    - test-dotnet-noinfra-snd1
    - test-dotnet-noinfra-snd2
    - test-dotnet-noinfra-snd3
variables:
    - test-dotnet-noinfra-APPLICATION-SECRET
```
